...
function calculateOpportunityScore(cf, sev, um, trendGrowthPercent, competitionProxy) {
  ...
  const normalizedScore = (rawScore * (1 - competitionProxy)) / 10;
  ...
  container.innerHTML = `
    <section class="card">
      ...
      <div class="grid-cards">
        <div class="card overview-card"><div class="metric-label">Total Reviews</div><div class="metric-value">${dataset.total_reviews.toLocaleString()}</div><div class="metric-note">Consumer signal depth</div></div>
        <div class="card overview-card"><div class="metric-label">Trend Growth %</div><div class="metric-value">${dataset.trend_growth_percent}%</div><div class="metric-note">Category momentum</div></div>
        <div class="card overview-card"><div class="metric-label">Top Complaint Bucket</div><div class="metric-value">${top.bucket}</div><div class="metric-note">Priority intervention zone</div></div>
        <div class="card overview-card highlight"><div class="metric-label">Highest Opportunity Score</div><div class="metric-value">${top.score}/10</div><div class="metric-note">Adjusted for competition pressure</div></div>
      </div>
    </section>
    ...
    <section class="card">
      ...
      ${ranked.map((row) => {
        ...
        return `
          <div class="bar-row">
            <div class="bar-label-wrap"><span>${row.bucket}</span><span>${row.score}/10</span></div>
            <div class="bar"><div class="bar-fill" style="width:${Math.min((row.score / maxScore) * 100, 100)}%"></div></div>
          </div>
        `;
      }).join("")}
    </section>
          ...
          <tbody>
            ${ranked.map((row, index) => `
              <tr>
                <td>${index + 1}</td>
                <td>${row.bucket}</td>
                <td><span class="score-chip">${row.score}/10</span></td>
        ...
        ${concepts.map((concept) => `
          <article class="card concept-card">
            ...
            <ul>
              ...
              <li><strong>Final Opportunity Score:</strong> ${concept.score}/10</li>
            </ul>
          </article>
        `).join("")}
      </div>
